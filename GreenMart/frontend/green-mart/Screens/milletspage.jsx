...
const Millets = [
  { id: 1, name: 'Bajra', price: 60, images: require('../assets/images/bajra.jpg') },
];

export default function MilletsPage(props) {
  const [cart, setCart] = useState([]);
  const [quantities, setQuantities] = useState(
    Millets.reduce((acc, item) => ({ ...acc, [item.id]: 0 }), {})
  );
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Fresh Millets</Text>
      {Millets.length === 0 ? (
        <Text style={{ textAlign: 'center', marginTop: 20 }}>No millets available.</Text>
      ) : (
        <FlatList
          data={Millets}
          keyExtractor={(item) => item.id.toString()}
          renderItem={renderMillets}
